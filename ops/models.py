...
class TSN(nn.Module):
    def __init__(self, num_class, num_segments, modality,
                 ...
                 temporal_pool=False, non_local=False, imgsz=224):
    ...
    def _prepare_base_model(self, base_model):
        ...
        if "Custom" in base_model:
            print(f"Loading resnet18 model using custom weights for image size of {self.imgsz}.")
            self.base_model = getattr(torchvision.models, base_model.rsplit('-')[-1])(False)
            in_features = self.base_model.fc.in_features
            self.base_model.fc = nn.Sequential(
                nn.Dropout(0.5),
                nn.Linear(in_features, 2))
            self.base_model.load_state_dict(torch.load(self.pretrain))
            ...
            print(f"Loading resnet18 model using {self.pretrain} weights for image size of {self.imgsz}.")
            self.base_model = getattr(torchvision.models, base_model)(weights="IMAGENET1K_V1" if self.pretrain=="imagenet" else None)
            if self.is_shift:
                print('Adding temporal shift...')
                from ops.temporal_shift import make_temporal_shift
                make_temporal_shift(self.base_model, self.num_segments,
